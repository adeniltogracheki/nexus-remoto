...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Ready", "準備完了"),
        ("Established", "接続完了"),
        ("connecting_status", "Nexus Remoto ネットワークに接続中..."),
        ...
        ("install_tip", "UAC の影響により、Nexus Remoto がリモートデスクトップ上で正常に動作しない場合があります。UAC を回避するには、下のボタンをクリックしてシステムに Nexus Remoto をインストールしてください。"),
        ("Click to upgrade", "アップグレード"),
        ("Configure", "設定"),
        ("config_acc", "リモートからあなたのコンピューターを操作するには、Nexus Remoto に「アクセシビリティ」権限を与える必要があります。"),
        ("config_screen", "リモートからあなたのコンピューターにアクセスするには、Nexus Remoto に「画面録画」の権限を与える必要があります。"),
        ("Installing ...", "インストール中..."),
        ("Install", "インストール"),
        ("Installation", "インストール"),
        ...
        ("Discovered", "発見済み"),
        ("install_daemon_tip", "起動時に Nexus Remoto を開始するには、システムサービスをインストールする必要があります。"),
        ("Remote ID", "リモート ID"),
        ("Paste", "貼り付け"),
        ...
        ("android_input_permission_tip1", "この Android デバイスをリモートコンピューターからマウスやタッチで操作するには、Nexus Remoto に「ユーザー補助」からサービスの使用を許可する必要があります。"),
        ("android_input_permission_tip2", "次の端末設定ページに進み、「インストール済みアプリ」から「Nexus Remoto Input」を有効にしてください。"),
        ...
        ("Turned off", "オフになりました"),
        ("Language", "言語"),
        ("Keep RustDesk background service", "Nexus Remoto バックグラウンドサービスを維持"),
        ("Ignore Battery Optimizations", "バッテリーの最適化を無効にする"),
        ("android_open_battery_optimizations_tip", "この機能を使わない場合は、Nexus Remoto アプリの設定ページから「バッテリー」に進み、「制限しない」を選択してください。"),
        ...
        ("elevated_foreground_window_tip", "リモートデスクトップでフォーカスされているウィンドウの操作にはより高い権限が必要なため、マウスとキーボードが一時的に使用できなくなっています。リモートユーザーにウィンドウを最小化、または接続管理画面から権限を昇格するよう要求してください。この問題を回避するには、リモートコンピューターに Nexus Remoto をインストールしてください。"),
        ("Disconnected", "切断しました"),
        ("Other", "その他"),
        ...
        ("Show RustDesk", "Nexus Remoto を表示"),
        ("This PC", "この PC"),
        ("or", "または"),
        ("Elevate", "昇格"),
        ...
        ("software_render_tip", "Linux で NVIDIA 製のグラフィックカードを使用していると、接続後すぐにリモートウィンドウが閉じてしまう場合があります。オープンソースの Nouveau ドライバーに切り替えて、ソフトウェアレンダリングを使用するよう設定すると解決するかもしれません。(Nexus Remoto の再起動が必要です)"),
        ("Always use software rendering", "常にソフトウェアレンダリングを使用する"),
        ("config_input", "リモートコンピューターをキーボードで操作するには、Nexus Remoto に「入力監視」権限を与える必要があります。"),
        ("config_microphone", "リモートコンピューターと通話するには、Nexus Remoto に「音声録音」権限を与える必要があります。"),
        ...
        ("still_click_uac_tip", "リモートデスクトップユーザーが Nexus Remoto を実行する際に、UACを許可する必要があります。"),
        ...
        ("verify_rustdesk_password_tip", "Nexus Remoto のパスワードを確認する"),
        ...
        ("scam_text1", "もし、知らない相手から電話で Nexus Remoto のインストールやサービスの開始を依頼された場合、作業を進めずに、すぐに電話を切ってください。"),
        ...
        ("selinux_tip", "SELinuxが有効になっているため、Nexus Remoto が正常に動作しない可能性があります。"),
        ("Change view", "表示を変更"),
        ("Big tiles", "大きなタイル"),
        ("Small tiles", "小さなタイル"),
        ("List", "リスト"),
        ...
        ("preset_password_warning", "このエディションには、既定で固定パスワードが設定されています。このパスワードを知っているユーザーはあなたのデバイスを完全にコントロールできるため、そのような危険がある場合は直ちに Nexus Remoto をアンインストールして下さい！"),
        ...
        ("allow-only-conn-window-open-tip", "Nexus Remoto のウィンドウが開いている場合のみ接続を許可する"),
        ...
        ("floating_window_tip", "Nexus Remoto のバックグラウンドサービスを維持するために使用されます。"),
        ...
        ("About RustDesk", "Nexus Remoto について"),
        ...
        ("upgrade_remote_rustdesk_client_to_{}_tip", "リモート側の Nexus Remoto クライアントをバージョン {} 以上にアップグレードしてください！"),
        ...
        ("allow-insecure-tls-fallback-tip", "既定では Nexus Remoto は TLS を使用するプロトコルのサーバー証明書を検証します。\nこのオプションを有効化すると Nexus Remoto は検証の手順をスキップして、検証に失敗した場合の処理を続行します。"),
        ("Disable UDP", "UDP を無効化する"),
        ("disable-udp-tip", "TCP のみ使用するかどうかを制御します。\nこのオプションを有効化すると、Nexus Remoto は UDP 21116 を使用せずに TCP 21116 を使用するようになります。"),
        ("server-oss-not-support-tip", "注意: Nexus Remoto Server OSS にはこの機能が含まれていません。"),
